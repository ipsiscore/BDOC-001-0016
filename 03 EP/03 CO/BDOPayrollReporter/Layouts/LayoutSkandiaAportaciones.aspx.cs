...
{
    public partial class LayoutSkandiaAportaciones : System.Web.UI.Page
    {
                    ...
                    if (Utilities.IsNumeric(this.DropDownListPeriodo.SelectedValue))
                    {
                        ...
                        if (Utilities.ValidarAño(año))
                        {
                            if (Utilities.ValidarMes(mes))
                            {
                                ...
                                {
                                    file = TextHelper.RenderLayoutSkandiaAportaciones(año, mes, DateTime.Parse(this.TextBoxFechaDispesion.Text), tipo, !DropDownListEstadoNomina.SelectedValue.Equals("0") ? true : false);

                                    if (file.Length > 0)
                                    {
                                        this.LabelMensaje.Text = "";
                                        Response.Clear();
                                        string fileName = Utilities.GetName(ReportName.Layoutdeaportacionesskandia, año, mes);
