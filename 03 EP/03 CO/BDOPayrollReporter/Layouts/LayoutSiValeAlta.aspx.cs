...
{
    public partial class LayoutSiValeAlta : System.Web.UI.Page
    {
                ...
                if (Utilities.IsNumeric(this.DropDownListAño.SelectedValue))
                {
                    if (Utilities.IsNumeric(this.DropDownListPeriodo.SelectedValue))
                    {
                        ...
                        if (Utilities.ValidarAño(año))
                        {
                            ...
                            {
                                file = TextHelper.RenderLayoutSiValeAlta(año, mes);

                                if (file.Length > 0)
                                {
                                    this.LabelMensaje.Text = "";
                                    Response.Clear();
                                    Response.AddHeader("content-disposition", "attachment; filename=layout.txt");
