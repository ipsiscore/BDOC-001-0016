...
{
    public partial class LayoutCityBank484 : System.Web.UI.Page
    {
                ...
                if (Utilities.IsNumeric(this.DropDownListAño.SelectedValue))
                {
                    if (Utilities.IsNumeric(this.DropDownListPeriodo.SelectedValue))
                    {
                        ...
                        if (Utilities.ValidarAño(año))
                        {
                            ...
                            {
                                if (Utilities.IsDate(this.TextBoxFechaDispesion.Text))
                                {
                                    if (Utilities.IsNumeric(this.TextoConsecutivo.Text))
                                    {
                                        file = TextHelper.RenderLayoutCitybank484(año, mes,
                                                                DateTime.Parse(this.TextBoxFechaDispesion.Text),
                                                                tipo,
                                                                !DropDownListEstadoNomina.SelectedValue.Equals("0") ? true : false, chkEsPrueba.Checked,
                                                                Convert.ToInt32(TextoConsecutivo.Text));

                                        if (file.Length > 0)
                                        {
                                            this.LabelMensaje.Text = "";
                                            Response.Clear();
                                            string fileName = Utilities.GetName(ReportName.Layoutdecitybank484, año, mes);
                                        ...
                                        this.LabelMensaje.Text = "No hay un consecutivo válido.";
                                }
                                else
